 ...
 *
 * <p> Most of the methods in this class are simple set-wise binary operators:
 * for example, calculate the intersection of two locations. </p>
 *
 * This class provides helpful methods for set-wise manipulation of Location
 * objects.
 *
...
final public class LocationTools {
     ...
     *
     * <p> The union will be a Location instance that contains every index
     * contained by either locA or locB. </p>
     *
    ...
    /**
     * Return the intersection of two locations. <p> The intersection will be a
     * Location instance that contains every index contained by both locA and
     * locB. </p>
     *
     * @param locA the first Location
     * @param locB the second Location
    ...
    /**
     * Return an ordered list of non-overlapping blocks of a location (singleton
     * list for contiguous locations, empty list for empty locations)
     *
     * Note that from this point of view, contiguous circular locations aren't
     ...
     * <code>bl</code> of a block containing
     ...
     * <code>point</code>, if it exists, or the block preceeding it.
     */
     ...
     * merged in a single Location. <p> Two locations can merge if they contain
     * at least one index of one beside one index of the other. </p>
     *
     * @param locA the first Location
     * @param locB the second Location
    ...
    /**
     * Determines whether the locations overlap or not. <p> Two locations
     * overlap if they contain at least one index in common. </p>
     *
     * @param locA the first Location
     * @param locB the second Location
    ...
    public static boolean overlaps(Location locA, Location locB) {
        ...
        if (locA.isContiguous() && locB.isContiguous()) {
            ...
            return !((locA.getMax() < locB.getMin())
    ...
    /**
     * Return
     * <code>true</code> iff all indices in
     * <code>locB</code> are also contained by
     * <code>locA</code>.
     *
    ...
    /**
     * Return whether two locations are equal. <p> They are equal if both a
     * contains b and b contains a. Equivalently, they are equal if for every
     * point p, locA.contains(p) == locB.contains(p). </p>
     *
     * @param locA the first Location
     * @param locB the second Location
    ...
    public static boolean areEqual(Location locA, Location locB) {
        ...
        if (locA.isContiguous()) {
            return (locA.getMin() == locB.getMin())
     ...
     * locList. <p> If the list is empty then Location.empty will be produced.
     * If it is just one element long, then this will be returned as-is. If
     * there are multiple locations then they will be sorted and then used to
     * construct a CompoundLocation.
     *
     * @param locList a List<Location>, where each element is a contiguous
     * location.
    ...
    /**
     * Create a Location instance from the sorted list of contiguous locations
     * in locList. <p> If the list is empty then Location.empty will be
     * produced. If it is just one element long, then this will be returned
     * as-is. If there are multiple locations then they will be sorted and then
     * used to construct a CompoundLocation.
     *
     * @param locList a List<Location>, where each element is a contiguous
     * location.
     * @return a new Location instance
    ...
    /**
     * The n-way union of a Collection of locations. Returns a Location which
     * covers every point covered by at least one of the locations in
     * <code>locs</code>
     *
     * @param locs A collection of locations.
     * @return A union location
     * @throws ClassCastException if the collection contains non-Location
     * objects.
     */
    ...
    /**
     * Return a contiguous Location from min to max. <p> If min == max then a
     * PointLocation will be made, otherwise, a RangeLocation will be returned.
     *
    ...
    public static Location makeLocation2(int min, int max, boolean fuzzyStart, boolean fuzzyEnd, String type) {
        Location ret = null;
        if (fuzzyStart || fuzzyEnd) {            
            SimplePosition simplePosition = new SimplePosition(fuzzyStart, fuzzyEnd, min, max, "..");
            //SimplePosition simplePositionMax = new SimplePosition(fuzzyStart, fuzzyEnd, min, max, "..");
            ret = new SimpleRichLocation(simplePosition, 1);
        } else if (min == max) {
            ret = new PointLocation(min);
        } else if (min < max) {
            ret = new RangeLocation(min, max);
        } else if(min > max){
            SimplePosition minPos = new SimplePosition(false, false, min, min, null);
            SimplePosition maxPos = new SimplePosition(false, false, max, max, null);
            ret = new SimpleRichLocation(minPos, maxPos, 1, null , null);
        }
        return ret;
    }

    /**
     * A simple method to generate a RangeLocation wrapped in a
     * CircularLocation. The method will cope with situtations where the min is
     * greater than the max. Either of min or max can be negative, or greater
     * than the underlying sequence length. If min and max are equal a wrapped
     * point location will be made.
     *
     * @param min the "left" end of the location
     * @param max the "right" end of the location
     * @param seqLength the lenght of the sequence that the location will be
     * applied to (for purposes of determining origin).
     ...
     * CircularLocationTools.safeOperation returns true. For this to be true the
     * locations must have the same sequence length.
     *
     * BetweenLocations are handled in all cases. Overlap cases, such as
     * between, circular locations have indeterminate behavior.
     *
     * The default behavior is to not handle decorators. Any new decorators will
     * have to re-visit this method
     */
     ...
     *
     * <p>It is very common in biological sequences to represent locations on a
     * sequence and then reverse that sequence. This method allows locations in
     * the original coordinate space to be transformed int locations in the
     * reverse one.</p>
     *
     ...
     * points which are contains in location
     * <code>keep</code> but not in
     * <code>remove</code>.
     *
    ...
    /**
     * Return a contiguous location running from the minimum to the maximum
     * points of the specified location.
     *
     * @param loc a location
