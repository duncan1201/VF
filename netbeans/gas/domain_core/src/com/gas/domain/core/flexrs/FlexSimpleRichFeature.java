 ...
 * @since 1.5
 */
public class FlexSimpleRichFeature extends AbstractChangeable implements RichFeature {
    ...
    private String name;
    private int rank = FlexSimpleRichFeature.nextRank++; // Auto-rank!
     ...
     */
    public FlexSimpleRichFeature(FeatureHolder parent, Feature.Template templ) throws ChangeVetoException, InvalidTermException {
        if (parent == null) {
    ...
    protected FlexSimpleRichFeature() {
    }
...
//        System.out.println("SimpleRichFeature.getLocationSet-featureId:"+featureId+", locsSet:"+locsSet+", this:"+this+", getLocation:"+getLocation());
    ...
    void setLocationSet(Set locs) throws ChangeVetoException {
        ...
        if (locs.size() > 0) {
            ((RichLocation) location).setTerm(((RichLocation) locs.iterator().next()).getTerm());
        }
...
//        System.out.println("SimpleRichFeature.setLocation-location:"+location+(location instanceof RichLocation?", term:"+((RichLocation) location).getTerm():""));
    ...
    public Feature createFeature(Feature.Template ft) throws BioException, ChangeVetoException {
        ...
        RichFeature f;
        try {
            f = new FlexSimpleRichFeature(this.parent, ft);
        } catch (InvalidTermException e) {
    ...
    /**
     * {@inheritDoc} Features are equal when they have the same rank, parent,
     * type, and source. Features which are not instance of RichFeature are
    ...
    public boolean equals(Object o) {
        ...
        if (!this.sourceTerm.equals(fo.getSourceTerm())) {
            return false;
        }
        return true;
    }
    ...
    /**
     * {@inheritDoc} Features are sorted first by rank, then parent, type, and
     * source. If both parents are not comparable then this part of the sorting
     * is skipped. Features which are not instance of RichFeature are given a
     * rank of zero.
     */
    ...
    /**
     * {@inheritDoc} Form: "(#rank) parent:type,source(location)"
     */
