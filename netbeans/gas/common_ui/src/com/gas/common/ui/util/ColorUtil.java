...
public class ColorUtil {
    ...
    public static <T> T getPerceivedBrightness(Color c, Class<T> clazz) {
        ...
        return getPerceivedBrightness(c, clazz, ALGO.X);

    }

    public static <T> T getPerceivedBrightness(Color c, Class<T> clazz, ALGO algo) {
        T ret = null;
        Color retColor = null;
        double pB;
        if (algo == ALGO.REC_709) {
            pB = 0.2126 * c.getRed() + 0.7152 * c.getGreen() + 0.0722 * c.getBlue();
        } else if (algo == ALGO.REC_601) {
