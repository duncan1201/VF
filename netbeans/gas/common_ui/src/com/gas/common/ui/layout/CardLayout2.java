...
/**
 * A
 * <code>CardLayout</code> object is a layout manager for a container. It treats
 ...
 * to a
 * <code>CardLayout</code> object is the visible component when the container is
 * first displayed.
 * <p>
 * The ordering of cards is determined by the container's own internal ordering
 * of its component objects.
 * <code>CardLayout</code> defines a set of methods that allow an application to
 * flip through these cards sequentially, or to show a specified card. The
 * {@link CardLayout#addLayoutComponent} method can be used to associate a
 * string identifier with a given card for fast random access.
 *
 * @version %I% %G%
 ...
 * @since JDK1.0
 */
public class CardLayout2 implements LayoutManager2,
        Serializable {
     ...
     * names. The object specified by
     * <code>constraints</code> must be a string. The card layout stores this
     * string as a key-value pair that can be used for random access to a
     * particular card. By calling the
     * <code>show</code> method, an application can display the component with
     * the specified name.
     *
    ...
    public void addLayoutComponent(String name, Component comp) {
        synchronized (comp.getTreeLock()) {
            ...
            for (int i = 0; i < vector.size(); i++) {
                if (((CardLayout2.Card) vector.get(i)).name.equals(name)) {
                    ((CardLayout2.Card) vector.get(i)).comp = comp;
                    return;
                }
            }
            vector.add(new CardLayout2.Card(name, comp));
        }
    }
    ...
    public void removeLayoutComponent(Component comp) {
        synchronized (comp.getTreeLock()) {
            for (int i = 0; i < vector.size(); i++) {
                if (((CardLayout2.Card) vector.get(i)).comp == comp) {
     ...
     * <code>parent</code> container is reshaped to be the size of the
     * container, minus space for surrounding insets, horizontal gaps, and
     * vertical gaps.
     *
     * @param parent the parent container in which to do the layout
     * @see java.awt.Container#doLayout
    ...
    public boolean contains(Container parent, String name) {
        Set<String> names = getNames(parent);
        return names.contains(name);
    }

    public Set<String> getNames(Container parent) {
        ...
        synchronized (parent.getTreeLock()) {
            ...
            for (int i = 0; i < ncomponents; i++) {
                CardLayout2.Card card = (CardLayout2.Card) vector.get(i);
                ret.add(card.name);
            }
        }
        return ret;
     ...
     * <code>addLayoutComponent</code>. If no such component exists, then
     * nothing happens.
     *
     * @param parent the parent container in which to do the layout
        ...
        if (f.defaulted("vector")) {
            ...
            if (tab != null && !tab.isEmpty()) {
                for (Enumeration e = tab.keys(); e.hasMoreElements();) {
                    ...
                    vector.add(new CardLayout2.Card(key, comp));
                    if (comp.isVisible()) {
