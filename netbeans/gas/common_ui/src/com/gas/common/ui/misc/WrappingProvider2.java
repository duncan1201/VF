 ...
 * content mapping can be configured by custom
 * <code>IconValue</code>s and
 ...
 * <p>
 */
public class WrappingProvider2 extends ComponentProvider<WrappingIconPanel2> implements RolloverRenderer {
     ...
     *
     */
    public WrappingProvider2() {
        this((ComponentProvider<?>) null);
    }

    /**
     * Instantiates a WrappingProvider with default wrappee, configured to use
     * the wrappeeStringValue. Uses the given IconValue to configure the icon.
     *
     ...
     */
    public WrappingProvider2(IconValue iconValue, StringValue wrappeeStringValue) {
        this(iconValue, wrappeeStringValue, true);
    }

    /**
     * Instantiates a WrappingProvider with default wrappee. Uses the given
     * IconValue to configure the icon.
     *
     * @param iconValue the IconValue to use for configuring the icon.
     */
    public WrappingProvider2(IconValue iconValue) {
        this(iconValue, null);
    }

    /**
     * Instantiates a WrappingProvider with default wrappee configured with the
     * given StringValue.
     *
     * PENDING: we have a slight semantic glitch compared to super because the
     * given StringValue is <b>not</b> for use in this provider but for use in
     * the wrappee!
     *
     * @param wrappeeStringValue the StringValue to use in the wrappee.
     */
    public WrappingProvider2(StringValue wrappeeStringValue) {
        this(null, wrappeeStringValue);
    }

    /**
     * Instantiates a WrappingProvider with the given delegate provider for the
     * node content. If null, a default LabelProvider will be used.
     *
     * @param delegate the provider to use as delegate
     */
    public WrappingProvider2(ComponentProvider<?> delegate) {
        this(delegate, true);
    }

    /**
     * Instantiates a WrappingProvider with the given delegate provider for the
     * node content and unwrapUserObject property. If the delegate is null, a
     * default LabelProvider will be used.
     *
     * @param delegate the provider to use as delegate
     * @param unwrapUserObject a flag indicating whether this provider should
     * auto-unwrap the userObject from the context value.
     */
    public WrappingProvider2(ComponentProvider<?> delegate, boolean unwrapUserObject) {
     ...
     * auto-unwrap the userObject from the context value.
     */
    public WrappingProvider2(IconValue iv, ComponentProvider<?> delegate, boolean unwrapUserObject) {
     ...
     * auto-unwrap the userObject from the context value.
     */
    public WrappingProvider2(IconValue iv, StringValue delegateStringValue, boolean unwrapUserObject) {
        this(iv, (ComponentProvider<?>) null, unwrapUserObject);
        getWrappee().setStringValue(delegateStringValue);
    }

    /**
     * Sets the given provider as delegate for the node content. If the delegate
     * is null, a default LabelProvider is set.<p>
     *
    ...
    /**
     * Sets the unwrapUserObject property. If true, this provider replaces a
     * context value of type XXNode with its user object before delegating to
     * the wrappee. Otherwise the value is passed as-is always.<p>
     *
    ...
    /**
     * Returns a boolean indicating whether this provider tries to unwrap a
     * userObject from a tree/table/node type value before delegating the
     * context.
     *
    ...
    /**
     * Returns the value as it should be passed to the delegate. If the
     * unwrapUserObject property is true, tries return a userObject as
     * appropriate for the value type. Returns the given value itself, ff the
     * property is false or the type does not support the notion of
     * userObject<p>
     *
     ...
     */
    @Override
    public WrappingIconPanel2 getRendererComponent(CellContext context) {
        if (context != null) {
     ...
     *
     * Subclasses may override but must guarantee to return the original value
     * for restoring.
     *
...
//    }
    @Override
    protected WrappingIconPanel2 createRendererComponent() {
        WrappingIconPanel2 ret = new WrappingIconPanel2();
        ret.setOpaque(true);
        return ret;
    }
     ...
     * {@inheritDoc} <p>
     *
     * Overridden to fallback to the default icons supplied by the context if
     * super returns null.
     *
     */
    @Override
