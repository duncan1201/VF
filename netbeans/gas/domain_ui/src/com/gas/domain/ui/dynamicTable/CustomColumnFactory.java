 ...
 * can be set per-application (before creating an JXTable) or per-table (before
 * setting the model). <p>
 *
 * Additional features:
 * <ul>
...
public class CustomColumnFactory extends ColumnFactory {
    ...
    /**
     * {@inheritDoc} <p>
     * Overridden to not create columns for the model columns which are listed
     * in the excludes.
     */
    @Override
    ...
    public void configureTableColumn(TableModel model,
        ...
        String colName = model.getColumnName(columnExt.getModelIndex());
        if(colName.equals(DynamicTableModel.NAME_COL) || colName.equals(DynamicTableModel.DESC_COL)){
            columnExt.setCellEditor(new JXTable.GenericEditor());
        }
        ...
        Comparator<?> comparator = getComparator(columnExt.getIdentifier());
        columnExt.setComparator(comparator);
        Object prototypeValue = getPrototypeValue(columnExt.getIdentifier());
        columnExt.setPrototypeValue(prototypeValue);
...
//    </snip>
    ...
    private void configureTitle(TableColumnExt columnExt) {
        String id = columnExt.getIdentifier().toString();
        columnExt.setTitle(NbBundle.getMessage(CustomColumnFactory.class, id));
    }
