 ...
 * @author dq
 */
public class InternalOligoTmCalSettingsPanel extends TmCalSettingsPanel{

    static final String PRIMER_INTERNAL_DNA_CONC = UserInput.PRIMER_INTERNAL_DNA_CONC;
    static final String PRIMER_INTERNAL_SALT_MONOVALENT = UserInput.PRIMER_INTERNAL_SALT_MONOVALENT;
    static final String PRIMER_INTERNAL_DNTP_CONC = UserInput.PRIMER_INTERNAL_DNTP_CONC;
    static final String PRIMER_INTERNAL_SALT_DIVALENT = UserInput.PRIMER_INTERNAL_SALT_DIVALENT;
    ...
    public static final String[] NAMES = {PRIMER_INTERNAL_DNA_CONC, 
    ...
    public void updateUserInputFromUI(UserInput userInput) {
        ...
        userInput.set(PRIMER_INTERNAL_DNA_CONC, _dnaConc);
        userInput.set(PRIMER_INTERNAL_SALT_MONOVALENT, _saltMonovalent);
        userInput.set(PRIMER_INTERNAL_DNTP_CONC, _dntpConc);
        userInput.set(PRIMER_INTERNAL_SALT_DIVALENT, _saltDivalent);
    ...
    public void populateUI(UserInput userInput) {
        String _dnaConc = userInput.get(PRIMER_INTERNAL_DNA_CONC);
        if (!_dnaConc.isEmpty()) {
            dnaConc.setValue(Float.parseFloat(_dnaConc));
        }

        String _saltMonovalent = userInput.get(PRIMER_INTERNAL_SALT_MONOVALENT);
        if (!_saltMonovalent.isEmpty()) {
            saltMonovalent.setValue(Float.parseFloat(_saltMonovalent));
        }

        String _saltDivalent = userInput.get(PRIMER_INTERNAL_SALT_DIVALENT);
        if (!_saltMonovalent.isEmpty()) {
            saltDivalent.setValue(Float.parseFloat(_saltDivalent));
        }

        String _dntpConc = userInput.get(PRIMER_INTERNAL_DNTP_CONC);
        if (!_saltMonovalent.isEmpty()) {
