...
public class LabelSeenTrackLayout implements LayoutManager2, IFtrTrackLayout {
    ...
    public void layoutContainer(Container parent) {
        ...
        List<IShape> arrows = ft.getArrows();
        if (arrows.isEmpty()) {
            return;
        }
        ILayoutHelper layoutHelper = LayoutHelperFinder.find((Feture)arrows.get(0).getData());
        maxY = 0;
        ...
        int minimumHeight = layoutHelper.getMinimumHeight(arrows.get(0));
        ...
        for (int i = 0; i < arrows.size(); i++) {
            ...
            while (isOccupied) {
                int y = j * minimumHeight - 1;
                y = Math.max(j, 0);
                int width = layoutHelper.getDesiredWidth(current, current.getTextLoc(), displayRect, totalPos);

                arrowRect = new Rectangle(desiredStartPixel, y, width, minimumHeight);
    ...
    public String getFetureType() {
        return IFtrTrackLayout.REGION_LAYOUT;
    }
}
